...
def forward_prop_dropout(X: np.ndarray, parameter: dict[str, np.ndarray], keep_prob: float) -> tuple[np.ndarray, tuple[np.ndarray, ...]]:
...
def backprop_dropout(X: np.ndarray, Y: np.ndarray, cache: tuple[np.ndarray, ...], keep_prob: float):
    ...
    dA2 = np.dot(W3.T, dZ3)
    dA2 = np.multiply(dA2, D2)  # dropout mask
    dA2 = dA2 / keep_prob
    ...
    dA1 = np.dot(W2.T, dZ2)
    dA1 = np.multiply(dA1, D1)  # dropout mask
    dA1 = dA1 / keep_prob
